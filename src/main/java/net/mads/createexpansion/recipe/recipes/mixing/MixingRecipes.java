package net.mads.createexpansion.recipe.recipes.mixing;

import com.google.gson.JsonObject;
import net.minecraft.data.CachedOutput;
import net.minecraft.data.PackOutput;

import java.util.List;
import java.util.concurrent.CompletableFuture;

import static net.mads.createexpansion.recipe.recipes.CreateRecipeJson.*;

public final class MixingRecipes {
    private MixingRecipes() {
    }

    public static void build(List<CompletableFuture<?>> futures, CachedOutput output, PackOutput.PathProvider recipes) {
        // save(futures, output, recipes, "mixing/test/bronze_dust", mixing("create_expansion:copper_dust", "create_expansion:tin_dust", "create_expansion:bronze_dust", "heated"));
    }

    private static JsonObject mixing(String inputA, String inputB, String output, String heatRequirement) {
        JsonObject json = new JsonObject();
        json.addProperty("type", "create:mixing");
        json.add("ingredients", ingredients(item(inputA), item(inputB)));
        json.add("results", results(result(output)));
        json.addProperty("heat_requirement", heatRequirement);
        return json;
    }
}
